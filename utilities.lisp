...
(defgeneric http:copy-stream (input-stream output-stream &key length)
  #+(or)
  (:method ((input-stream http:input-stream) (output-stream stream) &key (length (1- most-positive-fixnum)))
    (let* ((count 0))
      (declare (type fixnum count))
      (if (chunga:chunked-stream-input-chunking-p input-stream)
        (flet ((read-chunked-byte (stream)
          ...
          (loop for byte = (read-chunked-byte input-stream)
                when byte
        ...
        (let ((stream (chunked-stream-stream input-stream)))
          (loop for byte = (read-byte stream)
                unless (null byte)
                do (write-byte byte output-stream)
                until (>= (incf count) length)
                finally (when (and byte (listen stream))
                          (http:request-entity-too-large "Limit of ~d bytes exceeded." length)))))
